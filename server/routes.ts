...
<div class="container">
    ...
    try {
      ...
      await pool.query(
        ...
        ON CONFLICT (user_id, day) DO UPDATE SET
          ...
          cardio_min = CASE WHEN EXCLUDED.cardio_skipped THEN NULL ELSE COALESCE(EXCLUDED.cardio_min, daily_log.cardio_min) END,
          ...
          cardio_start_time = CASE WHEN EXCLUDED.cardio_skipped THEN NULL ELSE COALESCE(EXCLUDED.cardio_start_time, daily_log.cardio_start_time) END,
          cardio_end_time = CASE WHEN EXCLUDED.cardio_skipped THEN NULL ELSE COALESCE(EXCLUDED.cardio_end_time, daily_log.cardio_end_time) END,
          lift_start_time = CASE WHEN EXCLUDED.lift_skipped THEN NULL ELSE COALESCE(EXCLUDED.lift_start_time, daily_log.lift_start_time) END,
          lift_end_time = CASE WHEN EXCLUDED.lift_skipped THEN NULL ELSE COALESCE(EXCLUDED.lift_end_time, daily_log.lift_end_time) END,
          lift_min = CASE WHEN EXCLUDED.lift_skipped THEN NULL ELSE COALESCE(EXCLUDED.lift_min, daily_log.lift_min) END,
          lift_working_min = CASE WHEN EXCLUDED.lift_skipped THEN NULL ELSE COALESCE(EXCLUDED.lift_working_min, daily_log.lift_working_min) END,
          zone1_min = CASE WHEN EXCLUDED.cardio_skipped THEN NULL ELSE COALESCE(EXCLUDED.zone1_min, daily_log.zone1_min) END,
          zone2_min = CASE WHEN EXCLUDED.cardio_skipped THEN NULL ELSE COALESCE(EXCLUDED.zone2_min, daily_log.zone2_min) END,
          zone3_min = CASE WHEN EXCLUDED.cardio_skipped THEN NULL ELSE COALESCE(EXCLUDED.zone3_min, daily_log.zone3_min) END,
          zone4_min = CASE WHEN EXCLUDED.cardio_skipped THEN NULL ELSE COALESCE(EXCLUDED.zone4_min, daily_log.zone4_min) END,
          zone5_min = CASE WHEN EXCLUDED.cardio_skipped THEN NULL ELSE COALESCE(EXCLUDED.zone5_min, daily_log.zone5_min) END,
          lift_z1_min = CASE WHEN EXCLUDED.lift_skipped THEN NULL ELSE COALESCE(EXCLUDED.lift_z1_min, daily_log.lift_z1_min) END,
          lift_z2_min = CASE WHEN EXCLUDED.lift_skipped THEN NULL ELSE COALESCE(EXCLUDED.lift_z2_min, daily_log.lift_z2_min) END,
          lift_z3_min = CASE WHEN EXCLUDED.lift_skipped THEN NULL ELSE COALESCE(EXCLUDED.lift_z3_min, daily_log.lift_z3_min) END,
          lift_z4_min = CASE WHEN EXCLUDED.lift_skipped THEN NULL ELSE COALESCE(EXCLUDED.lift_z4_min, daily_log.lift_z4_min) END,
          lift_z5_min = CASE WHEN EXCLUDED.lift_skipped THEN NULL ELSE COALESCE(EXCLUDED.lift_z5_min, daily_log.lift_z5_min) END,
  ...
  app.get("/api/debug/workout-session/:sessionId", async (req: Request, res: Response) => {
    try {
      const userId = getUserId(req);
      const sessionId = req.params.sessionId as string;
      const audit = await getWorkoutSessionAudit(pool, sessionId, userId);
      res.json(audit);
    } catch (err) {
      console.error("workout session audit error:", err);
      res.status(500).json({ error: "Internal server error" });
    }
  });

  app.get("/api/debug/workout-today", async (req: Request, res: Response) => {
    try {
      const userId = getUserId(req);
      const tz = req.query.timezone as string | undefined;
      const day = (req.query.day as string) || (tz ? toUTCDateString(new Date().toISOString(), tz) : new Date().toISOString().slice(0, 10));
      ...
      res.json({
        ...
        summary: {
          lift_logged: dailyLog.rowCount > 0 && dailyLog.rows[0]?.lift_done === true,
          strength_set_count: strengthSets.rowCount,
          bridge_set_count: bridgeSets.rowCount,
          session_count: sessions.rowCount,
        },
      });
    } catch (err) {
      console.error("workout today audit error:", err);
      res.status(500).json({ error: "Internal server error" });
    }
  });
  ...
  app.post("/api/canonical/workouts/upsert-session", async (req: Request, res: Response) => {
    try {
      ...
      if (p.source === "workout_game" && p.end_ts && wt === "strength") {
        persistWorkoutDerivedState(pool, {
          kind: "session_close",
          ...
          workingMinutes: p.working_minutes ?? null,
        }).catch(() => {});
      } else if (p.session_id && p.start_ts) {
  ...
  app.get("/api/intel/strength/trend", async (req: Request, res: Response) => {
    ...
    try {
      const r = await fetch(`${INTEL_BASE}/strength/trend?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`);
      return intelProxy(r, res);
    } catch (err: any) {
      console.error("GET /api/intel/strength/trend error:", err);
      return res.status(502).json({ error: "Network failure reaching lifting-intel", details: String(err) });
    }
  });

  app.get("/api/intel/strength/day", async (req: Request, res: Response) => {
    res.set("Cache-Control", "no-store");
    if (!INTEL_BASE) return res.status(503).json({ error: "LIFTING_INTEL_BASE_URL not configured" });
    const date = (req.query.date as string) || new Date().toISOString().slice(0, 10);
    try {
      const r = await fetch(`${INTEL_BASE}/strength/day?date=${encodeURIComponent(date)}`);
      return intelProxy(r, res);
    } catch (err: any) {
  ...
  app.get("/api/intel/muscle-map", async (req: Request, res: Response) => {
    ...
    const date = (req.query.date as string) || new Date().toISOString().slice(0, 10);
    try {
      const r = await fetch(`${INTEL_BASE}/muscle/day?date=${encodeURIComponent(date)}`);
      return intelProxy(r, res);
    } catch (err: any) {
