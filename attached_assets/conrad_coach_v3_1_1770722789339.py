...
def lean_gain_ratio_rolling(e: pd.DataFrame, window_days: int = 14) -> pd.Series:
    """
    Rolling lean gain ratio over time.

    For each day i, look back `window_days` calendar days (inclusive) using available rows,
    select the first and last row within that window that have lean_mass, and compute:
        (lean_mass_last - lean_mass_first) / (weight_last - weight_first)

    Returns a Series aligned to e.index with NaN where insufficient data.
    """
    if "lean_mass_lb" not in e.columns:
        return pd.Series([pd.NA] * len(e), index=e.index, dtype="float")

    ratios = []
    for i in range(len(e)):
        end_day = e["day"].iloc[i]
        start_day = end_day - pd.Timedelta(days=window_days-1)
        window = e[(e["day"] >= start_day) & (e["day"] <= end_day)].dropna(subset=["lean_mass_lb"])
        if len(window) < 2:
            ratios.append(pd.NA)
            continue
        w0 = float(window["morning_weight_lb"].iloc[0])
        w1 = float(window["morning_weight_lb"].iloc[-1])
        lm0 = float(window["lean_mass_lb"].iloc[0])
        lm1 = float(window["lean_mass_lb"].iloc[-1])
        dw = w1 - w0
        if abs(dw) < 0.1:
            ratios.append(pd.NA)
            continue
        ratios.append((lm1 - lm0) / dw)

    return pd.Series(ratios, index=e.index, dtype="float")
...
def build_dashboard() -> None:
    ...
    e = df_entries(data)
    if e.empty:
        print("No entries logged yet. Run: python conrad_coach_v3_1.py log")
        return
    f = df_fitbit(data)
    ...
    # Lean gain ratio snapshot + rolling series
    ratio = lean_gain_ratio_14d(e)
    merged["lean_gain_ratio_14d"] = ratio
    roll = lean_gain_ratio_rolling(e, window_days=14)
    # clamp for readability (BIA noise can create silly ratios)
    merged["lean_gain_ratio_14d_roll"] = roll.clip(lower=-1.0, upper=2.0)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python conrad_coach_v3_1.py [checklist|log|report|dashboard|fitbit_ingest <csv_path>]")
        sys.exit(1)
    ...
    if cmd == "checklist":
        ...
        if len(sys.argv) < 3:
            print("Usage: python conrad_coach_v3_1.py fitbit_ingest <path_to_fitbit_export.csv>")
            sys.exit(1)
