...
"""
Conrad Coach v2
- Daily checklist (shake-time template + anchors)
- Daily logging (sleep, water, weights, waist, training, cardio)
- Optional Fitbit Charge 6 CSV ingest (exported from Fitbit dashboard)
- Weekly dashboard:
    * outputs/conrad_dashboard.csv (daily rows + rolling avgs)
    * outputs/conrad_weight.png (weight trend)
    * outputs/conrad_waist.png (waist trend, if available)
    * outputs/conrad_activity.png (steps/cardio, if available)
- Adaptive diet adjustment:
    * Target gain: +0.25 to +0.5 lb/week
    * If none: +75–100 kcal/day
    * If >0.75 lb/week: -100 kcal/day
- "If cardio minutes > X then auto +Y carbs" logic (training-day fuel guardrail)
- Planned deload week flag to prevent misdiagnosis
"""
...
BASELINE = {
    ...
    # If daily cardio minutes > threshold, suggest adding carbs on that day.
    "cardio_fuel": {
        "threshold_min": 45,   # X
...
def weekly_delta_7day_avg(e: pd.DataFrame) -> Optional[float]:
    if len(e) < 14:
        return None
    w7 = e["morning_weight_lb"].rolling(7, min_periods=7).mean()
    last = w7.iloc[-1]
    prev = w7.iloc[-8]
...
def build_dashboard() -> None:
    ...
    e = df_entries(data)
    if e.empty:
        print("No entries logged yet. Run: python conrad_coach_v2.py log")
        return
    f = df_fitbit(data)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python conrad_coach_v2.py [checklist|log|report|dashboard|fitbit_ingest <csv_path>]")
        sys.exit(1)
    ...
    if cmd == "checklist":
        ...
        if len(sys.argv) < 3:
            print("Usage: python conrad_coach_v2.py fitbit_ingest <path_to_fitbit_export.csv>")
            sys.exit(1)
