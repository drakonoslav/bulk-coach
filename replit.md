...
- **Cardio Regulation Engine**: Unified 3×2 regulation architecture for cardio. Schedule Stability: alignment (start time deviation from planned), consistency (SD of start times across 7 sessions), recovery (return to schedule after drift event). Outcome uses productive-time model: productiveMin=z2+z3, cardioTotalMin=sum(z1..z5). Adequacy=100×productive/planned (capped 110), Efficiency=100×productive/total, Continuity=100×(1−z1/total). If zones missing, fallback to spanMinutes for total but all outcome scores null. Default schedule: 06:00–06:40 (40 min). File: `server/cardio-regulation.ts`.
- **Lift Regulation Engine**: Unified 3×2 regulation architecture for lifting. Schedule Stability: alignment, consistency, recovery (identical logic to cardio). Outcome: adequacy=100×actualMin/plannedMin (capped 110), efficiency=100×workingMin/actualMin, continuity=100×(1−idleMin/actualMin). workingMin from `lift_working_min` column (manual entry); if null, efficiency/continuity are null. Default schedule: 17:00–18:15 (75 min). File: `server/lift-regulation.ts`.
