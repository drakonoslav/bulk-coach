...
- **Androgen Oscillator Engine**: (`server/oscillator-engine.ts`) Implements the 3-layer convergent rhythm control model from the 10-year oscillator philosophy. Computes three 0–100 scores — Acute Readiness (HRV ratio, RHR delta, sleep duration/regularity, soreness, bodyweight stability; 50% weight), Tissue-Resource (7d calorie adequacy, dietary adherence, 14d BW/waist/FFM trends; 30% weight), and Endocrine-Seasonal (28d HRV/RHR trends vs 56d baseline, waist:weight ratio, FFM trend, cardio variety; 20% weight) — combining into a composite with 5 tier interpretations (Peak Anabolic/Strong Build/Controlled Output/Low-Stress Volume/Recovery). Outputs a daily prescription: day type (SURGE/BUILD/RESET/RESENSITIZE), cardio zone, lift expression, and macro targets. Rendered at the top of the Vitals tab with expandable per-layer breakdowns. Route: `GET /api/oscillator`. Future inputs: libido/drive rating, pump quality, sunlight exposure, subjective motivation.
