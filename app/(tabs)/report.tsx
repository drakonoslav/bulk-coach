                          ...
                          {debugSchedExpanded && (
                            <View style={{ backgroundColor: "#60A5FA08", borderRadius: 6, padding: 8, marginTop: 4, marginBottom: 4 }}>
                              ...
                              <Text style={{ fontSize: 10, fontFamily: "Rubik_400Regular", color: Colors.textTertiary, lineHeight: 16 }}>
                                {`planBed = ${saD?.plannedBedTime ?? "—"} [${sb?.debugSources?.planBedSource ?? "?"}]\nplanWake = ${saD?.plannedWakeTime ?? "—"} [${sb?.debugSources?.planWakeSource ?? "?"}]\nactualBed = ${saD?.observedBedLocal ?? "—"} [${sb?.debugSources?.actualBedSource ?? "?"}]\nactualWake = ${saD?.observedWakeLocal ?? "—"} [${sb?.debugSources?.actualWakeSource ?? "?"}]\ndataDay = ${sb?.debugSources?.dataDay ?? "—"}\nplannedSleepMin = ${sb?.plannedSleepMin ?? "—"}  TIB = ${sb?.timeInBedMin ?? "—"}  TST = ${sb?.estimatedSleepMin ?? "—"}\ntibTstPath = ${sb?.debugSources?.tibTstPath ?? "?"}`}
                              </Text>
